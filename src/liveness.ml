...
let perform_analysis graph =
  ...
  let set_preds info_map =
    Label.M.fold (fun l _ map ->
        Label.S.fold (fun n_l map' ->
  ...
  let rec kildall (info_map, working_set) =
    if Label.S.is_empty working_set then info_map
        ...
        let ws =
          if Register.S.subset in_ old_in_ then ws else Label.S.union ws info.pred
        in
        kildall (info_map, ws)
      end
  in
